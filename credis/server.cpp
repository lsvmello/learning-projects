...
static bool try_one_request(Conn *conn) {
  ...
  // pac the response into the buffer
...
static void connection_io(Conn *conn) {
  // walked up by poll, update the idle timer
  // by movind conn to the end of the list.
  conn->idle_start = get_monotonic_usec();
...
static void process_timers() {
  // te extra 1000us is for the ms resolution of poll()
  ...
  while (!dlist_empty(&g_data.idle_list)) {
    ...
    if (next_us >= now_us + 1000) {
      // not ready, the extra 1000us is for the ms resolution of poll() break;
      break;
    }
...
int main() {
  ...
  thread_pool_init(&g_data.tp, k_num_threads);
